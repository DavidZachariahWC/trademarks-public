...
def multi_filter_search(filter_tree: Dict, page: int = 1, per_page: int = 10) -> Dict[str, Any]:
    ...
    def build_query_recursive(node: Dict, session: Session) -> Query:
        ...
        if 'strategy' in node:
            ...
            # Combine them
            if operator == 'AND':
                # Start from a base_query, filter by intersection of serial_numbers
                combined_query = base_query(session)
                for sq in subqueries:
                    # Convert each subquery to a sub-select of serial_number 
                ...
                # Start from base_query, filter by OR of subquery membership
                or_conditions = []
                for sq in subqueries:
                    serial_number_subselect = select(sq.with_entities(CaseFile.serial_number).subquery().c.serial_number)
    ...
    # ----------------------------------------------------------------------
    # 2) Gather scoring subqueries: (serial_number, score).
    #    We'll union them + group by serial_number to get max(score).
    # ----------------------------------------------------------------------
    scoring_subqueries = []
    ...
    # ----------------------------------------------------------------------
    # Handle an empty filter tree gracefully
    # ----------------------------------------------------------------------
    if not filter_tree:
        return {
            'results': [],
    ...
    try:
        ...
        # C. Build a brand-new final_results_query that:
        #    - References CaseFile + CaseFileHeader
        #    - Filters by matching_serials_subq
        #    - Outer-joins scoring_agg if present
        final_results_query = (
            session.query(
        ...
        results = []
        for row in rows:
            # If there's scoring, row includes 8 columns; otherwise 7
            if scoring_agg is not None:
                (serial_number,
                 registration_number,
                 mark_identification,
